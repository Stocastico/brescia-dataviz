...
✅ **Questa sezione è chiusa** (23 settembre 2026): non c'è più nessuna riga 🙋
aperta nella tabella qui sotto. La licenza è scelta (§3.3), la sorgente di
Pages è a posto dal 4 settembre 2026, dal 19 settembre 2026 fare merge su
`main` **è** pubblicare — non c'è più un clic a parte da dare — e i testi del
sito sono stati riletti. La decisione di disegno sulla tavolozza condivisa con
`donostia-dataviz` (riga 10 qui sotto) è stata presa il 7 settembre 2026:
approvata com'è.
...
| ✗ 4 | ~~**Scaricare gli open data del Comune di Brescia**~~ — **la voce cade** (4 settembre 2026): il portale è dismesso, non irraggiungibile ([perché](dati/SCARICHI-MANUALI.md) §2) | non era la nostra rete: `comune.brescia.it/opendata` risponde `410 Gone`. I dataset sono su `dati.lombardia.it`, quindi 🤖; il turismo cittadino 2005–2013 non è migrato e resta perduto | — | niente, tranne l'estensione indietro della storia sul turismo (`#turismo`), che va dichiarata come non disponibile |
...
| ✅ 7 | ~~**Rileggere i testi prima di pubblicare**~~ (§8) — **fatta il 23 settembre 2026** | era il tuo nome sopra | 1 h | niente più |
...
| ✅ 10 | ~~**Dire se la tavolozza va riallineata con `donostia-dataviz`**~~ — **decisa il 7 settembre 2026: va bene com'è.** I due progetti restano una collana con una tavolozza che diverge, e la regola dei colori resta scritta qui invece che là. ⚠️ La decisione arriva nello stesso giorno in cui la regola ha **finito lo spazio** al nono colore (`--indaco`, 29 gradi dal vicino contro i 40 della coppia più stretta): approvare l'esistente non rimanda quel problema, lo lascia esattamente dov'è, e il decimo colore è stato **deciso** invece che calcolato, il 7 settembre 2026: `--bruno` `#6d4e22`, la tinta dell'ambra a luminosità 28 %, cioè differenziata sull'asse che restava. È anche **l'ultimo** che quell'asse concede, perché verso il chiaro si passa sotto la soglia di contrasto AA ([`sito/README.md`](sito/README.md) §Lo stile) | — | — | niente |
...
provinciale e trimestrale, e le tre tabelle dell'università. La 🙋 6 e la
🙋 10 sono state chiuse **da una decisione** invece che da un lavoro.

✅ **E le ultime due sono state chiuse anche loro** (19 e 23 settembre 2026): la
🙋 9 (pubblicare) è caduta il 19, quando il merge su `main` è diventato la
pubblicazione stessa; la 🙋 7 (rileggere i testi) è stata fatta il 23. **Questa
tabella non ha più nessuna riga 🙋.**
...
| Tabelle tidy | ✅ **39 CSV** in [`dati/processed/`](dati/README.md), versionati — quattro vengono dall'OMI (quotazioni in grana zona e comunale, compravendite comunali e provinciali) più il **deflatore** `indice_prezzi.csv`, le tre tabelle università e le due su INPS/INAIL; manca solo `migrazioni_comuni.csv`, che resta fuori da git per scelta (§2.1) |
| Asse «casa e prezzi» | ✅ **completo**: i dati (quotazioni OMI 2004–2025 in due grane, compravendite 2011–2025), l'analisi (`casa_e_prezzi.py`, §4) e la **storia** — `#casa` nel sito. Prezzo reale −30,8 % in ventun anni mentre i volumi fanno +134,5 % |
| Analisi | ✅ **sedici** script in [`analysis/`](analysis/README.md): velocità di cambio, quadranti, autocorrelazione, tipologia, le due economie, la scomposizione del capoluogo, la rottura del 2020, il confronto fra le 107 province, la convergenza dei redditi su Bergamo, la scomposizione demografica, il background migratorio, l'aria e il clima, il turismo confrontato con le altre province, e da settembre la **casa** — prezzi contro volumi, le zone del capoluogo, i 203 comuni quotati |
| Storie scelte | ✅ **dieci**, scritte e pubblicate nel documento narrativo — la storia sulle due economie (`#economie`) corregge quelle che la precedono, quella sul turismo (`#turismo`) è l'unica che nessuno si aspettava, quella sulla casa (`#casa`) è la sola in cui il risultato dipende da come si misura. Le candidate rimaste stanno in `BRIEF.md` |
...
| Deploy | ✅ automatico su `main` (§7): la **costruzione** gira a ogni push (test, cifre, sito, artefatto) e, se è verde, la **pubblicazione** la segue — dal 19 settembre 2026 il cancello manuale è caduto. Il lancio a mano resta, ma solo per costruire senza pubblicare da un ramo. La sorgente di Pages è su «GitHub Actions» dal 4 settembre 2026 |
...
fatte, dieci storie sono scritte in un sito che si costruisce **e si pubblica**
da solo a ogni push su `main` (19 settembre 2026), e i testi sono stati riletti
(23 settembre 2026): **non resta più nessuna riga 🙋**. Il lavoro tecnico che
resta è tutto facoltativo: i download manuali. Il pannello interattivo, che era
la voce grossa, **è stato fatto** a settembre 2026 (§6.1).
...
- **tutti e diciannove gli indicatori** del registro, raggruppati per tema, e
  non i quindici che le storie citano nel racconto. La lista non è scritta nel modello:
...
| ✅ ~~Pubblicare~~ (§7) | fatta | il 19 settembre 2026: da allora un merge su `main` **è** la pubblicazione, se la costruzione è verde |
| ✅ ~~Rileggere i testi del sito~~ | fatta | il 23 settembre 2026 |
...
| ✅ ~~Una storia su aria e clima (asse 4)~~ | fatta | `analysis/aria_e_clima.py` e la storia sull'aria e sul clima del sito (`#aria`). Ne è uscita anche MET-16, e la pioggia è entrata nel racconto **proprio perché** non dà segnale |
...
| ✅ ~~Portare il turismo nel sito come nuova storia~~ | fatta | «La decima provincia turistica d'Italia» (`#turismo`), con lo sciame delle 107 province, i diciassette anni di presenze divise fra clienti italiani e stranieri, e lo scarto fra le due fonti disegnato invece che raccontato |
| ✅ ~~Analisi e storia sulla casa~~ (§4) | fatta | `analysis/casa_e_prezzi.py` e la storia sulla casa del sito (`#casa`): prezzi reali −30,8 % contro volumi +134,5 % |
...
| ✅ ~~Nona e decima storia (background migratorio, confronto)~~ | fatte | «Cinquantatremila nati qui» (`#origini`) e «Brescia è diversa?» (`#diversa`), chiuse il 10 settembre 2026: il sito è a **dieci** storie |
...
Ogni riga di questa tabella era una voce **tocca a te**; a partire dal 23
settembre 2026 sono tutte fatte. Quello che resta sotto, in "se hai tempo", non
è più un elenco di cose in sospeso: è dove guardare se un giorno si vuole
...
- **Un'undicesima storia**, se un'altra idea dalle candidate di `BRIEF.md` sembra
  matura (§4 elenca le piste non ancora valutate — demografia d'impresa, lavoro
  da casa, isola di calore, Caffaro);
- **L'asse 2 fino in fondo**, scaricando in locale `migrazioni_comuni.csv`
  (venti minuti, §2.1) per la congiunta completa, oltre alle due marginali già
  versionate;
- **Gli affitti e lo stock abitativo vuoto**, che restano sull'asse casa
  (§4, "Cosa resta su questo asse");
- **Il confronto con Bergamo**, il controllo naturale rimasto aperto per MET-9
  (§4, "Sul metodo").
